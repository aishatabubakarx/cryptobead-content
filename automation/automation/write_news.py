...
def get_todays_slot_and_author():
    state = load_json(ROTATION_STATE_PATH, {"date": "", "slot": 0, "rotation_index": 0})
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")

    if state["date"] != today:
        state = {"date": today, "slot": 0, "rotation_index": state.get("rotation_index", 0)}
...
def main():
    ...
    slot, author = get_todays_slot_and_author()
